...
First, I will create a linear model.
...
A method that can be used to improve the results of adjusted p-values is multiple hypothesis testing! This helps to control for patient variability, by taking both the patients' results AND the number of patients into account, instead of just the patients' results.

First, the linear model needs to be updated so that it now takes into consideration the patients' results and the number of patients.
...
When I was using Limma, I could not get any significantly diffeerentially expressed genes, before or after multiple hypothesis testing. 

I then re-read my paper and noticed that they also performed gene diffeerential analysis, but used a different package than Limma, as the data seemed to follow a negative binomal distribution (this is why understanding the entirety of the paper is important!). Limma works well on data that follows a linear distribution, which as explained by my paper, my data does not. EdgeR is another package that can be used for differential expression, and is great for data that follows a negative binomial distribution - just like mine!

I will also confirm that my data follows a negative binomial distribution:
...
First, I will use the edgeR glmQLFTest to fit my dataset. I will fit my dataset using the multiple hypothesis test like I did with Limma, where I used both the number of patients and the pateints' data in an attempt to control for the patients' variability.
...
Next, I will plot the data on a heatmap, and look at the differences between patients before surgery (T0) and after surgery (T3).
...
I am going to continue to use edgeR as it allowed me to find genes that passed correction. While using edgeR, I will be finding the downregulated and upregulated genes from my dataset.
...
I was able to find that there are 732 upregulated genes, and 709 downregulated genes! Now I will find the ensembl gene IDs of these genes and save them in text files, which I will put into g:profiler so that I can see what datasets match those genes best.
...
Now that I have text files with all of the necessary genes, I will use these text files in g:profiler and put images of the results below.
...
The amount of genesets from each annotation data set for each query performed are shown below.
