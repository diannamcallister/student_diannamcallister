...
I have always been interested in health, and maintaining a healthy lifestyle. I always knew being healthy (whether that means eating healthily or being active / working out) had amazing benefits on your physical, mental, and emotional health. So a study that focused on health, and how a procedure like bariatric surgery can improve someone's physical health, which hopefully leads to improving their physical, mental, and emotional health.
...
**Amount of GEO samples that use this Platform:** `length(current_gpl_info$sample_id)`
...
* There are 23354 genes (unsure about duplicates just yet!)
* There are gene names, Ensemble gene IDs, and 12 different test cases (two situations per gene)
...
Firstly, as mentioned above, some of the genes are missing Ensembl gene IDs. This is a large issue and I had lots of difficulty trying to salvage as many genes as I could that were missing the Ensembl gene IDs.
...
```
There are `r length(na_gene_ids)` genes without ensembl gene ids! I will try to improve this.
...
*Method 2: Use entrezgeneids on genes that begin with LOC*
In my dataset there are quite a few genes that begin with the letters LOC. Dr. Isserlin suggested that if the LOC is removed, these ids can be used as entrez gene ids! I then separated all gene ids that began with LOC and performed a query:
```{r}
LOC_indexes <- grep("^LOC", still_na) # Find all gene names beginning with LOC
LOC_names <- still_na[LOC_indexes]
no_LOC_names <- gsub("^LOC", "", LOC_names) # Remove all of the 
...
for (val in left_na) {
  if (val %in% aliasSymbol$alias_symbol) {
    ...
    all_new_symbols <- c(all_new_symbols, proper_symbol) #for next step
  }
}
...
This is the last method I could find. Even though there are still some genes that are missing ensembl ids, I will leave them in as they do have some form of identificaiton, though the gene ids used may be aliases or older hugo symbols.

Finally, to actually find the HUGO symbols that map to all of thse ensembl gene ids:
```{r}
all_HUGO <- getBM(attributes = c("ensembl_gene_id","hgnc_symbol"),
                   filters = c("ensembl_gene_id"),
                   values = PM_exp_filtered$`Ensembl Gene ID`,
                   mart = ensembl_grch37)
PM_exp_filtered$"HUGO_symbol" <- NA # Add column to dataset
...
```
 ...
 If I remember correclty, the gene names listed here look an awful lot like the list of gene names who origianlly were missing ensemble gene ids! Let's check:
...
```
 ...
 Wow! Most of the genes that are duplicates originally had no ensembl gene ids! As this is less than 3% of my dataset, I am going to leave all of these values in. I don't feel comfortable removing genes, especially when I am unsure of the fact that the genes that are duplicated are being given the correct ensembl gene ids after all the methods I had to go through!
...
I then used a denstiy plot as it is similar to a histogram, but you are able to easily view the distribution of data over a continuous interval of patient's expression of the genes.
```{r echo = FALSE}
...
We can see that there will be minor modifications to the dataset will be minor but will still have a slight impact (as seen from the norm.factors column. 
```{r echo = FALSE}
...
From this plot, I can automatically see that all patients, wether before or after surgery had very similar interquartile ranges, with a mean around 4. There seemed to be quite a few outliers, many on the more negative side, indicating much lower expression occurred slightly more frequently than very high expression.
```{r echo = FALSE}
...
The trend line (blue) approaches the common one (red) from the left side of the plot until about 0.3, in which the lines seem to overlap and follow a very similar trend. Also, the trend line never surpasses 0.5 of biological coefficient of variation, which demonstrates that my dataset does not have very much biological variation and most of my samples have similar expression of the genes.
...
It seems that the genes with a higher mean expression level have an increased level of variance, whereas those that have a lower mean expression have a decreased level of variance. This could be due to the fact that the larger expression level values are so larger that even a small change could be represented as a much larger variance (as we are using log10 scale).
...
Starting at x= 0, there seems to be lots of convering at the y = 0. This indicates that the gene values for PM01 at both T0 and T3 relative to the surgery were extremely similar, however there were some genes (where lots of (before x = 0) where either a T0 there was very little expression, whereas at T3 there was lots of expression, or vice versa. Therefore, some genes that were observed had very little difference before and after surgery, whereas other genes had large amounts of differences before and after surgery.
